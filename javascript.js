...
function makeCells(numDiv) {
    for (d=0; d<(numDiv*numDiv); d++) {
        ...
        createdCells.setAttribute('style', `height:${dimensions()}px; width:${dimensions()}px;`)
...
padCells.forEach((padCell)=>{padCell.addEventListener('mouseover', event =>{
    let paintColor = document.getElementById("palette").value;
        padCell.setAttribute('style', `height:${dimensions()}px; width:${dimensions()}px; background:${paintColor};`);
    }) 
})
}
...
function erase() {
    const padCells = document.querySelectorAll('div.cells');
    padCells.forEach((padCell)=>{padCell.addEventListener('mouseover', event =>{
        padCell.setAttribute('style', `height:${dimensions()}px; width:${dimensions()}px; background:white;`);
    }) 
})
}
...
function rainbow() {
    const padCells = document.querySelectorAll('div.cells');
    padCells.forEach((padCell)=>{padCell.addEventListener('mouseover', event =>{
        padCell.setAttribute('style', `height:${dimensions()}px; width:${dimensions()}px; background:${randomColor()};`);
    }) 
})
}
